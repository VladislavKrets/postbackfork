...
public class PostbackController {
    ...
    public String sendPostback(@PathVariable(name = "postback_url") String postbackUrl, Model model){
        ...
        List<TrackerEntity> trackers = mySQLDao.getTrackers();
        if (postBackEntity.getAfid() != 0) {
            for (TrackerEntity entity : trackers) {
                if (postBackEntity.getAfid() == entity.getPrefix()) {
                    switch (entity.getPrefix()) {
                        case 101: affiseHandler(postBackEntity); break;
                        case 102: binomHandler(postBackEntity, entity.getDomain());
                    }
                }
            }
        }
    ...
    private PostBackEntity fillPostback(Map<String, String> parameters) {
        ...
        String afid = parameters.get("clickid").length() > 3 ? clickid.substring(0, 3) : null;
        if (afid != null && afid.matches("\\d\\d\\d")) {
            postBackEntity.setClickId(clickid.substring(3, clickid.length()));
            postBackEntity.setAfid(Integer.parseInt(afid));
        }
        else {
            postBackEntity.setClickId(clickid);
            postBackEntity.setAfid(0);
        }
